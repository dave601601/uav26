...
- **Startup-tumble flake.** When the sim launches, gz Harmonic's OdometryPublisher reports the drone's orientation as `q=(0, 0, 1, 0)` (= 180° about Y) for the first few physics steps before the simulation has settled. The firmware controller reads this as "drone upside-down" and commands a big righting torque, which actually tumbles the drone on the ground into a wedged orientation it can't escape. **Mitigated** by `fc_sim_node`'s sanity gate (only publishes nonzero motor speeds when the IMU shows the drone within ±45° of level, for 20 consecutive ticks). Drone now stays at identity orientation through startup.

- **Ground-contact constraint flake (UNSOLVED).** Even with motors commanded at ω=620 rad/s (gz-verified via `gz topic -e`, F_total≈17 N vs weight 11.8 N), the drone refuses to lift off if it touches the ground first. DartSim's contact constraint solver appears to absorb the upward force. Reproducible with the hover_pub takeoff burst at thrust_norm=0.75 sustained for 20 s. Only succeeds when gz happens to catch the drone mid-fall during initialization — pure timing luck. Possible fixes to try next session: lower `mu` / friction on the body collision; use sphere instead of box for body collision; remove body collision entirely and rely on rotor collisions only; or accept it and have line_tracer drive the drone through whatever takeoff sequence works for the real airframe.
